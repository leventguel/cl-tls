(defpackage :tls-aes128-gcm
  (:use :cl :shared-utils :tls-aes-utils :tls-aes-ghash :tls-aes128)
...
(defun aes128-gcm-encrypt (plaintext key iv &optional (aad #()) (taglen 128) (aad-len 0) (ctlen 0) verbose)
  ...
  (let* ((expanded-key
    ...
    (let* ((ciphertext (gctr-128 expanded-key init-counter plaintext))
	   (aad-len (length aad))
...
(defun aes128-gcm-decrypt (ciphertext key iv tag &optional (aad #()) (taglen 128) (aad-len 0) (ctlen 0) verbose)
  ...
  (let* ((expanded-key
    ...
    (let* ((plaintext (gctr-128 expanded-key init-counter ciphertext))
	   (aad-len (length aad))
	   (ctlen (length ciphertext))
	   (len-block (concatenate '(vector (unsigned-byte 8))
