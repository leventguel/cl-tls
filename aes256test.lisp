...
(defun trace-ecb-encryption (plaintext expanded-key)
  "Trace AES-256 ECB encryption stages for each 16-byte block in the plaintext."
...
(defun run-test ()
  "Runs AES-256 test vector and prints match result."
  (let* ((key-str "2b7e151628aed2a6abf7158809cf4f3c")
         ...
         (output   (aes256-ecb-encrypt pt key)))
...
(defparameter *test-ciphertext*
  (aes256-ecb-encrypt *test-plaintext* *test-key*))

;; Decrypt it
(defparameter *recovered-plaintext*
  (aes256-ecb-decrypt *test-ciphertext* *test-key*))
...
(defparameter *enc-block*
  (aes256-ecb-encrypt-block *padded* *key*))

(defparameter *expanded-key*
  (expand-key-256 (copy-seq *key*)))

(defparameter *dec-block*
  (aes256-ecb-decrypt-block *enc-block* *expanded-key*))
