...
(defun run-all-tests ()
  ...
  (run-test "HMAC-SHA256 Test 1: key = #x0b * 20, msg = 'Hi There'"
            (make-array 20 :element-type '(unsigned-byte 8) :initial-element #x0b)
            (string-to-bytes "Hi There"))

  ;; RFC 4231 Test Case 2
  (run-test "HMAC-SHA256 Test 2: key = 'Jefe', msg = 'what do ya want for nothing?'"
            (string-to-bytes "Jefe")
            (string-to-bytes "what do ya want for nothing?"))

  ;; RFC 4231 Test Case 3
  (run-test "HMAC-SHA256 Test 3: key = #xaa * 20, msg = #xdd * 50"
