...
(load "~/clocc/src/ssl/sha512.lisp") ; Adjust path as needed

(defun hmac-sha512 (key message)
  (let* ((block-size 128)
         (key (if (> (length key) block-size)
                  (sha512 (coerce key '(vector (unsigned-byte 8))))
                  key))
...
(defun hmac-sha512-hex (key message)
  (bytes-to-hex (hmac-sha512 key message)))
