...
(defun my-sha512 (key message)
  (hmac-sha512-hex key message)) ; Your implementation

(defun run-test (label key message)
  (let* ((expected (string-downcase (ironclad-hmac-sha512 key message)))
         (actual   (string-downcase (my-sha512 key message))))
...
(defun run-all-tests ()
  ...
  (run-test "HMAC-SHA512 Test 1: key = 0x0b * 20, msg = 'Hi There'"
            (make-array 20 :element-type '(unsigned-byte 8) :initial-element #x0b)
            (string-to-bytes "Hi There"))

  ;; RFC 4231 Test Case 2
  (run-test "HMAC-SHA512 Test 2: key = 'Jefe', msg = 'what do ya want for nothing?'"
            (string-to-bytes "Jefe")
            (string-to-bytes "what do ya want for nothing?"))

  ;; RFC 4231 Test Case 3
  (run-test "HMAC-SHA512 Test 3: key = 0xaa * 20, msg = 0xdd * 50"
