...
(defun trace-ecb-encryption (plaintext expanded-key)
  "Trace AES-128 ECB encryption stages for each 16-byte block in the plaintext."
...
(defun run-test ()
  ...
  (let* ((key-str "2b7e151628aed2a6abf7158809cf4f3c")
         ...
         (output   (aes128-ecb-encrypt pt key)))
...
(defparameter *test-ciphertext*
  (aes128-ecb-encrypt *test-plaintext* *test-key*))

;; Decrypt it
(defparameter *recovered-plaintext*
  (aes128-ecb-decrypt *test-ciphertext* *test-key*))
...
(defparameter *enc-block*
  (aes128-ecb-encrypt-block *padded* *key*))

(defparameter *expanded-key*
  (expand-key-128 (copy-seq *key*)))

(defparameter *dec-block*
  (aes128-ecb-decrypt-block *enc-block* *expanded-key*))
