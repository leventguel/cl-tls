(defpackage :tls-aes192-mac
  (:use :cl :shared-utils :tls-aes-utils :tls-aes192)
  (:export :cbcmac-aes192 :cmac-aes192))

(in-package :tls-aes192-mac)
...
(defun generate-subkeys (key)
  (let* ((L (aes192-ecb-encrypt-block
...
(defun cbcmac-aes192 (message key &optional (block-size 16))
  "Computes CBC-MAC using AES-192. Message must be a multiple of block-size."
  ...
  (let* ((expanded-key (expand-key-192 key))
    ...
    (loop for i from 0 below (length message) by block-size
          ...
          do (setf previous-block
                   (aes192-cbc-encrypt-block block previous-block expanded-key)))
    previous-block)) ;; Final block is the MAC

(defun cmac-aes192 (msg key tlen)
  "Computes CMAC using AES-192 and returns tlen-byte MAC."
...
(defun test-cbcmac-aes192 ()
  (let* ((key (hex-string-to-byte-vector "8E73B0F7DA0E6452C810F32B809079E5"))
         (message (make-array 32 :element-type '(unsigned-byte 8)
                              :initial-contents (loop for i below 32 collect i)))
         (mac (cbcmac-aes192 message key)))
    (format t "~%CBC-MAC: ~{~2,'0X~^ ~}~%" (coerce mac 'list))))

(defun test-cmac-aes192 ()
  (let* ((key (hex-string-to-byte-vector "8E73B0F7DA0E6452C810F32B809079E5"))
         (msg #()) ; empty message
         (expected (hex-string-to-byte-vector "D17DD0A0E8BEEA74397A7DFA5AEB7A99")) ; example 192-bit CMAC
         (mac (cmac-aes192 msg key 16)))
    (format t "~%CMAC192 Test Result: ~A~%" (equalp mac expected))
